...
ZarishDocs is a browser-only, zero-cost AI research lab. The current repository already contains the static UI shell, the Worker proxy scaffold, the source-of-truth research/config docs, and source files under `src/`.
...
There is a check script but no full linter is configured yet:
- `npm run check` → `node --check` each shipped browser-side module under `src/` (`api.js`, `app.js`, and the agent modules)

### Formatting

Prettier is the formatting tool recorded in the architecture docs. Do not reformat files you did not touch.
...
- Agent logic lives in `src/agents/` as separate service modules: `profiler.js`, `research.js`, `architect.js`, and `writer.js`.
- Shared API/client boundaries live in `src/api.js` and the `DEFAULT_MODELS` routing table there.
- Client-side state is intentionally lightweight and browser-native: no framework, no state library, no fetch calls from render code, no backend service up front.
...
- Unit tests should cover pure logic where it exists in the current agents (`profileIdea`, `validateIdea`, research payload validation, and result shapes).
