...
- **Local file access (ADR-003):** native File System Access API (`showSaveFilePicker`) on Chromium desktop, `<a download>` fallback elsewhere — no `browser-fs-access` dependency. Surfaced on first load, never a silent failure.
