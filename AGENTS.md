...
- **Worker deploy (from `worker/`):** `wrangler secret put GEMINI_API_KEY`, then `wrangler deploy`. The Worker is also auto-deployed on every push to `main` by the Cloudflare **Workers Builds** integration, which runs `wrangler deploy` from the repo **root** — the root `wrangler.toml` (mirror of `worker/wrangler.toml` with `main = "worker/index.js"`) exists so that build succeeds. Keep the two configs in sync when editing vars. App: `wrangler pages deploy . --project-name zarishdocs --branch main --commit-dirty=true` from the repo **root** — `index.html`/`styles.css`/`sw.js`/`manifest.webmanifest` live there, not in `src/`.
- **Worker gotcha:** `worker/wrangler.toml` (and its root mirror) `ALLOWED_ORIGIN` is a comma-separated list that includes the loopback/localhost origins (local dev) plus `https://zarishdocs.pages.dev`. If you add a custom domain, append it to **both** configs and redeploy the Worker, or production browsers on that origin get a 403.
