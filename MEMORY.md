...
- 2026-08-11 — **Native file writes (ADR-003 revised):** dropped `browser-fs-access`. `src/file-writer.js` uses the File System Access API (`showSaveFilePicker`) on Chromium desktop and an `<a download>` fallback everywhere else — no dependency, same UX contract (surfaced on first load, never a silent failure).
