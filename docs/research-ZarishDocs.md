...
- **MCP (Model Context Protocol)** — the establishe
d standard for how an AI agent calls external tools/data sources, now on spec version 2026-07-28. It is usually server-side, but MCP servers can run as local subprocesses over stdio, which is relevant only if ZarishDocs ever ships a companion desktop/CLI layer — not needed for the pure browser MVP.
